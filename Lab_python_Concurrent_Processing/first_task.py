...
def structuring_of_output(func_object, files, executor):
    values = []
    columns = []
    result = list(executor.map(func_object, files))
    ...
    if 'max' in func_object.__name__:
        named_count = max([row[0] for row in named])
    else:
        named_count = min([row[0] for row in named])
    values.append(named_count)
    columns.append(named[[row[0] for row in named].index(named_count)][1])
